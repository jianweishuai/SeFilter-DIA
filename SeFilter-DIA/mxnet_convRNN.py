...
class Model(gluon.nn.Block):
    def __init__(self, **kwargs):
        ...
        self.convRNN = gluon.contrib.rnn.Conv2DRNNCell(input_shape=[1, 6, 85], 
    ...
    def forward(self, x):
        init_state = self.convRNN.begin_state(batch_size=batch_size, ctx=mx.gpu())
        state = init_state
        f1, state = self.convRNN(x, state)
		#print(f1.shape)
        return(self.out(f1))
...
def testing_auc(net, test_iter, loss, ctx):
    ...
    return testing_loss, test_auc, fpr, tpr, predit_score, test_precison, test_recall, test_f1_score, test_acc, test_confusionMatrix
...
def validating_auc(net, valid_iter, loss, ctx):
    ...
    return validating_loss, valid_auc, predit_score, fpr, tpr
...
def train_ch5(net, train_iter, valid_iter, test_iter, batch_size, trainer, ctx, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % 50 == 0 or epoch == num_epochs - 1:
            ...
            if valid_auc > best_valid_auc:
                ...
                testing_loss, test_auc, fpr_test, tpr_test, predict_score_test, test_precison, test_recall, test_f1_score, test_acc, test_confusionMatrix = testing_auc(net, test_iter, loss, ctx)
                if test_auc > best_test_auc:
                    ...
                    plt.savefig("./figure/convRNN/" + model_name +'_'+ Ad  +'_'+ "ROC.jpg")
                    ...
                    out_file = os.getcwd() + '/figure/convRNN/'+ Ad +'_'+'importantCriteria.txt'
    ...
    plt.savefig("./figure/convRNN/" + model_name +'_'+ Ad +'_'+ "_trainingloss.jpg")
